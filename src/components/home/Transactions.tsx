  ...
  return (
    <div className="flex flex-col container">
      <div className="flex justify-between bg-gray-900 p-4 rounded-t-lg text-gray-200">
        ...
        <div className={`flex text-3xl ${trntype === "CREDIT" ? "text-green-700" : "text-red-400"}`}>{trntype}</div>
        <div className={`flex justify-end text-5xl mr-10 ${trntype === "CREDIT" ? "text-green-700" : "text-red-400"}`}>Total: {totalValor}</div>
      </div>
      <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
          <tr className="text-2xl ">
            <th scope="col" className={`flex px-2 m-2 py-2 cursor-pointer rounded-lg ${trntype === "CREDIT" ? "text-green-700 bg-green-200" : "text-red-700 bg-red-300"}`} onClick={() => trntype === "CREDIT" ? setTrntype("DEBIT") : setTrntype("CREDIT")}>
        ...
        <tbody className={`${trntype === "CREDIT" ? "text-green-800" : "text-red-900"}`}>
